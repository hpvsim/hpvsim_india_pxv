...
def plot_single(ax, mres, to_plot, si, ei, color, label=None):
...
def plot_fig3(msim_dict):
    ...
    for pn, cov_val in enumerate(plot_coverage_arr):
        ...
        ax = plot_single(ax, mres, to_plot, si, ei, 'r', label='Baseline')

        # Plot adolescents
        adolescent_label = f'Adolescent: {cov_val} coverage'
        mres = msim_dict[adolescent_label]
        ax = plot_single(ax, mres, to_plot, si, ei, 'k', label='Adolescents only')
